...
object FileIO {
   ...
   * @param idsToTitles - the LinkedHashMap of document Ids to document titles
   */
  def printTitleFile(titleFile: String, idsToTitles: LinkedHashMap[Int, String]) {
   ...
   * @param idsToMaxCounts - the LinkedHashMap of document Ids to their Euclidean normalization
   * @param idsToPageRanks - the LinkedHashMap of document Ids to their PageRank
   */
  def printDocumentFile(documentFile: String,
                        idsToMaxCounts: LinkedHashMap[Int, Double],
                        idsToPageRanks: LinkedHashMap[Int, Double]) {
   ...
   * @param wordsToDocumentFrequencies - the LinkedHashMap of words to a LinkedHashMap of the document Ids they appear in along
   *                                   with their frequency in that document
   */
  def printWordsFile(wordsFile: String,
                     wordsToDocumentFrequencies: LinkedHashMap[String, LinkedHashMap[Int, Double]]) {
   ...
   * @param idsToTitle - LinkedHashMap of document Ids to their titles
   */
  def readTitles(titleIndex: String, idsToTitle: LinkedHashMap[Int, String]) {
  ...
  def readWords(wordIndex: String,
    ...
    while (line != null) {
      ...
      wordsToDocumentFrequencies(tokens(0)) = new LinkedHashMap[Int, Double]
   ...
   * @param idsToMaxFreqs - LinkedHashMap of document Ids to their Euclidean normalization
   * @param idsToPageRank - LinkedHashMap of document Ids to their pagerank
   */
  def readDocuments(documentIndex: String, idsToMaxFreqs: LinkedHashMap[Int, Double], idsToPageRank: LinkedHashMap[Int, Double]) {
